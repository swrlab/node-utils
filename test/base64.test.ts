...
describe('toBase64', () => {
	...
	it('should throw a TypeError if a number is passed', () => {
		...
		assert.throws(() => toBase64(123456789), { name: 'TypeError', code: 'ERR_INVALID_ARG_TYPE' })
	})
})
...
describe('parseBase64', () => {
	it('should throw a TypeError on undefined', () => {
		// @ts-expect-error - A number won't work, but is fine for testing.
		assert.throws(() => parseBase64(undefined), {
			name: 'TypeError',
			message: /The first argument must be of type/,
		})
	})
