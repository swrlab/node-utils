...
import { getEnv, getEnvBase64, getEnvBoolean, getEnvString } from './../src/env.ts'

describe('no env values available', () => {
	it('should throw an error for missing required values', () => {
		assert.throws(() => getEnv<string>('MISSING', { type: 'string', required: true }), {
			name: 'MissingEnvVarError',
		})
	})
	it('should throw an error for missing required values (even with defaultValue)', () => {
		assert.throws(() => getEnv<string>('MISSING', { type: 'string', required: true, defaultValue: 'hi' }), {
			name: 'MissingEnvVarError',
		})
	})
	...
	describe('functions with `required` set to `true` by default', () => {
		it('should throw an error for getEnvString when the value is missing', () => {
			assert.throws(() => getEnvString('MISSING'), { name: 'MissingEnvVarError' })
		})
		it('should throw an error for getEnvBase64 when the value is missing', () => {
			assert.throws(() => getEnvBase64('MISSING'), { name: 'MissingEnvVarError' })
		})
	})
...
describe('mocked env values', () => {
	...
	describe('getEnvJSON', () => {
		it('should throw a SyntaxError for an empty value', () => {
			assert.throws(() => getEnvBase64('EMPTY_JSON'), {
				name: 'Error',
				message: /EMPTY_JSON.*SyntaxError: /,
			})
		})
